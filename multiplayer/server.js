...
const WEAPONS_DATA = {
    he:     { id: 'he',     name: 'Odłamkowo-Burzący',  damage: 25, velocity: 140, blastRadius: 12, type: 'projectile' },
    ap:     { id: 'ap',     name: 'Przeciwpancerny',     damage: 40, velocity: 220, blastRadius: 0,  type: 'projectile' },
    heat:   { id: 'heat',   name: 'Kumulacyjny',         damage: 55, velocity: 160, blastRadius: 0,  type: 'projectile' },
    emp:    { id: 'emp',    name: 'EMP',                 damage: 0,  velocity: 150, blastRadius: 8,  type: 'projectile', effectDuration: 4.0 },
    smoke:  { id: 'smoke',  name: 'Dymny',               damage: 0,  velocity: 100, blastRadius: 15, type: 'projectile' },
    guided: { id: 'guided', name: 'Naprowadzany',        damage: 45, velocity: 100, blastRadius: 3,  type: 'missile', lifespan: 10.0 }
};
...
function fireWeapon(playerId, action) {
    ...
    if (weaponData.type === 'projectile') {
        ...
        const projectile = {
            ...
            velocity: weaponData.velocity, lifespan: 4.0,
        };
...
function fireMachineGun(playerId, action) {
    ...
    const bullet = {
        ...
        direction: direction, velocity: 200, lifespan: 2.0,
    };
                ...
                if (p.blastRadius > 0) {
                    ...
                    for (const playerId in gameState.players) {
                        ...
                        if (dist < p.blastRadius) {
                            ...
                            handleDamage(player, p.damage * damageFalloff, p.ownerId);
                        }
                    }
                     if (p.weaponId === 'emp') {
                        for (const playerId in gameState.players) {
        ...
        if(destroyed) {
             for (const playerId in gameState.players) {
                ...
                if (dist < m.blastRadius) {
                    ...
                    handleDamage(player, m.damage * damageFalloff, m.ownerId);
                }
            }
